...
需要多次调用，但不需要多次执行。（连续执行则移除上次执行）

```js
  debounce(fn, delay = 200, atBegin = true) {
    let timer = null; let last = 0; let during;
...
需要多次调用，也需要多次执行。（控制执行间隔）

```js
  throttle(fn, delay = 100, atBegin = true) {
