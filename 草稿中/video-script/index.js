 ...
 */
(() => {
  const videoScript = (getVideoDom = ( ) => window.document.getElementsByTagName('video')[0]) => {
    const videoDom = typeof getVideoDom === 'function' ? getVideoDom() : videoDom
    if (!videoDom ) {
    ...
    const formatSeconds = seconds => {
      ...
      const formatNum = num => Math.floor(num).toString().padStart(2, '0')
    ...
    const run = () => {
      ...
      document.body.addEventListener('keydown', (e) => {
        ...
        } else if (hasAllowCode(e.key, [KeyCodeMap.ArrowLeft, KeyCodeMap.ArrowRight])) {
          e.preventDefault()
          const gap = e.metaKey ? videoOps.getDuration() * config.mateGap : config.gap
